...
import { fetchSpfItems, getSpfBffOrigin } from "@/shared/api/spf";
...
function parseItemListQuery(
  ...
  const period_id =
    typeof rawPeriodId === "string" && rawPeriodId.trim().length <= 100
      ? rawPeriodId.trim()
      : undefined;
...
export default async function ItemsPage({ searchParams }: Props) {
  ...
  const result = await fetchSpfItems(cookieHeader, query, getSpfBffOrigin());
