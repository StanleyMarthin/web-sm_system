...
import { RequestsListShell } from "@/modules/requests/components/requests-list-shell";
import { ModuleUnavailableState } from "@/shared/ui/module-unavailable-state";

async function RequestsListPageContent() {
  const requestHeaders = await headers();
  const cookieHeader = requestHeaders.get("cookie") ?? "";

  const [
    { payload: woPayload, status: woStatus },
    { payload: prPayload },
    { payload: vendorPayload },
    { user },
  ] = await Promise.all([
    fetchWoGrid(cookieHeader, { limit: "100", viewMode: "all" }),
    fetchPrGrid(cookieHeader, { limit: "100", viewMode: "all" }),
    fetchVendorGrid(cookieHeader, { limit: "100", viewMode: "all" }),
...
export default function RequestsListPage() {
  return <RequestsListPageContent />;
}
