...
export function PeriodListShell({ rows, meta, access }: PeriodListShellProps) {
  const [isCreateOpen, setCreateOpen] = useState(false);
  const [notice, setNotice] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<PeriodStatusFilter>("DRAFT_REJECT");
  ...
  const draftRejectCount = rows.filter(
    (p) => p.workflow_status === "DRAFT" || p.workflow_status === "REJECTED",
  ).length;
  const waitingCount = rows.filter((p) => p.workflow_status === "WAITING_APPROVAL").length;
  const approvedCount = rows.filter((p) => p.workflow_status === "APPROVED").length;
  const publishedCount = rows.filter((p) => p.workflow_status === "PUBLISHED").length;

  // Filter rows based on status, unit, month, year, and search
  const filteredRows = rows.filter((period) => {
    // 1. Status Tab
    if (activeTab === "DRAFT_REJECT" && period.workflow_status !== "DRAFT" && period.workflow_status !== "REJECTED") {
      return false;
    }
    if (activeTab === "WAITING_APPROVAL" && period.workflow_status !== "WAITING_APPROVAL") {
      return false;
    }
    if (activeTab === "APPROVED" && period.workflow_status !== "APPROVED") {
      return false;
    }
    if (activeTab === "PUBLISHED" && period.workflow_status !== "PUBLISHED") {
      return false;
    }
      ...
      <div className="flex flex-wrap items-center gap-2.5 rounded border border-border bg-muted/30 p-2.5 dark:border-white/[0.06] dark:bg-white/[0.02]">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            id="filter-status"
            value={activeTab}
            onChange={(e) => setActiveTab(e.target.value as PeriodStatusFilter)}
