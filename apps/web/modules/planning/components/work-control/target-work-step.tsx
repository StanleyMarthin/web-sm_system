...
function recommendationBadgeStyle(rec: PlanningRecommendation): string {
  switch (rec) {
    case "SPK":
      return "border-success/30 bg-success/[0.06] text-success";
    case "SPK_WITH_SPL":
      return "border-primary/30 bg-primary/[0.06] text-app-accent-ink";
    case "HOLD":
    ...
    <div className="border border-border bg-card">
      <div className="border-b border-border px-4 py-3">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div>
            ...
            {onDelete && (
              <button
                type="button"
                onClick={onDelete}
                className="mt-1 text-[15px] font-mono text-destructive hover:text-destructive"
              >
                Hapus Target Ini
              </button>
            )}
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="border border-border px-2 py-0.5 font-mono text-[15px] uppercase tracking-[0.1em] text-muted-foreground">
              Risiko {formatRiskLabel(entry.riskLevel)}
            </span>
            <span
              className={`border px-2 py-0.5 font-mono text-[15px] uppercase tracking-[0.1em] ${recommendationBadgeStyle(recommendation)}`}
            >
              {statusLabel}
            </span>
          </div>
      ...
      <div className="grid gap-4 px-4 py-4 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <div className="mb-2">
            <label className="font-mono text-[15px] text-foreground">
            ...
            {shouldShowSearch && (
              <div className="border-b border-border px-3 py-2">
                <input
                  ...
                  className="h-8 w-full border border-border bg-card px-3 font-mono text-[14px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/40"
                />
              </div>
            )}
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      selected ? "bg-primary/[0.08]" : "hover:bg-accent",
                    ].join(" ")}
                  >
                    <span className="block font-mono text-[14px] text-foreground">{label}</span>
                    <span className="mt-1 block text-[15px] leading-4 text-muted-foreground">
        ...
        <div>
          <label className="mb-2 block font-mono text-[15px] text-foreground">
            Divisi pelaksana
          </label>
          <select
            ...
            className="h-8 w-full border border-border bg-background px-2.5 font-mono text-[14px] text-foreground outline-none"
          >
        ...
        <div>
          ...
          <input
            ...
            placeholder="0"
            className="h-8 w-full border border-border bg-background px-3 font-mono text-[14px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/40"
          />
          <div className="mt-2 flex flex-wrap gap-1.5">
            {[
              ...
              <button
                ...
                className="border border-border bg-muted px-2 py-1 font-mono text-[14px] text-foreground hover:bg-muted"
              >
                + {preset.label}
              </button>
            ))}
        ...
        <div>
          ...
          <input
            ...
            className="h-8 w-full border border-border bg-background px-3 font-mono text-[14px] text-foreground outline-none dark:[color-scheme:dark]"
          />
        </div>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {[
              ...
              <button
                ...
                className={[
                  "h-8 border px-2 font-mono text-[14px] uppercase tracking-[0.12em] transition-colors",
        ...
        <div className="flex items-end pb-1">
          <button
            ...
            className={[
              "inline-flex h-8 w-full items-center justify-center border px-4 font-mono text-[14px] uppercase tracking-[0.12em] transition-colors",
              entry.isHold
      ...
      {entry.divisionId > 0 && (
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <span
              className={`border px-2 py-1 font-mono text-[15px] uppercase tracking-[0.1em] ${recommendationBadgeStyle(recommendation)}`}
            >
              {statusLabel}
            </span>
          </div>
      ...
      <div className="border-t border-border px-4 py-3">
        <button
          ...
          className="font-mono text-[14px] uppercase tracking-[0.1em] text-muted-foreground"
        >
          {showDetails ? "Sembunyikan Detail Tambahan" : "Tampilkan Detail Tambahan"}
        </button>
        {showDetails && (
          <div className="mt-3">
            <label className="mb-1 block font-mono text-[14px] uppercase tracking-[0.1em] text-muted-foreground">
              Catatan (opsional)
            </label>
            <textarea
              rows={2}
              ...
              placeholder="..."
              className="w-full border border-border bg-background px-2.5 py-2 font-mono text-[15px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/45"
            />
          </div>
        )}
      </div>
    ...
    <div className="space-y-4">
            ...
            {units.map((u) => {
              ...
              return (
                <button
                  ...
                  className={[
                    "flex h-8 items-center gap-2 border px-3 font-mono text-[15px] transition-colors",
                    isActive
                ...
                >
                  ...
                  {hasIncomplete && (
                    <span className="flex h-1.5 w-1.5 rounded-full bg-destructive"></span>
                  )}
                </button>
              );
            })}
          ...
          {activeUnit && (
            <div className="space-y-4">
              <div className="border border-border bg-card px-4 py-4">
                ...
                <h3 className="mt-1 font-mono text-[14px] text-foreground">
                  {activeUnit.unitName}
                </h3>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <span className="inline-flex items-center border border-border bg-background px-2.5 py-1 font-mono text-[15px] uppercase tracking-[0.12em] text-muted-foreground">
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 border border-dashed border-border bg-muted py-3 font-mono text-[15px] text-foreground hover:bg-border"
              >
                + Tambah Divisi
              </button>
            </div>
      ...
      <div className="flex items-center justify-between">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex h-8 items-center gap-2 border border-border px-4 font-mono text-[14px] uppercase tracking-[0.12em] text-muted-foreground transition-colors hover:bg-border hover:text-foreground"
        >
          ← Kembali
        </button>
        <button
          ...
          className="inline-flex h-8 items-center gap-2 border border-primary/30 bg-primary/[0.06] px-4 font-mono text-[14px] font-bold uppercase tracking-[0.12em] text-app-accent-ink transition-colors hover:bg-primary/10 disabled:cursor-not-allowed disabled:opacity-40"
        >
          Lanjut ke Review →
        </button>
      </div>
