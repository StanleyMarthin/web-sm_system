...
function EmptyBlock({ message }: { message: string }) {
  return (
    <div className="rounded-xl border border-dashed border-white/[0.08] bg-white/[0.02] px-4 py-8 text-sm text-white/35">
      {message}
    </div>
  );
}
...
export function QaDashboardShell({ dashboard }: { dashboard: QaDashboardSummary }) {
  ...
  const donutStops = (() => {
    ...
    const colors = ["#f59e0b", "#38bdf8", "#34d399", "#f87171", "#a78bfa", "#fbbf24", "#94a3b8"];
    let start = 0;
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-xl border border-white/[0.06] bg-[#0b0d10] p-4">
          <div className="mb-4">
            <p className="text-sm font-semibold text-white">Top 5 Divisi Penyumbang Reject</p>
          </div>
                ...
                return (
                  <div key={item.divisionName} className="space-y-1.5">
                    <div className="flex items-center justify-between gap-3 text-sm">
                      <span className="text-white/75">{item.divisionName}</span>
                      <span className="font-semibold text-white">{item.rejectCount}</span>
                    </div>
                    <div className="h-2.5 rounded-full bg-white/[0.06]">
                      <div className="h-2.5 rounded-full bg-red-400" style={{ width: `${width}%` }} />
                    </div>
                  </div>
                );
              })}
        ...
        <div className="rounded-xl border border-white/[0.06] bg-[#0b0d10] p-4">
          <div className="mb-4">
            <p className="text-sm font-semibold text-white">Distribusi Area Masalah</p>
          </div>
          {dashboard.issueAreaDistribution.length > 0 ? (
            <div className="grid gap-4 md:grid-cols-[180px_1fr] md:items-center">
              <div className="mx-auto flex h-[180px] w-[180px] items-center justify-center rounded-full" style={{ background: donutStops }}>
                <div className="flex h-[112px] w-[112px] items-center justify-center rounded-full bg-[#0b0d10] text-center">
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.14em] text-white/35">Total</p>
                    <p className="mt-1 text-2xl font-semibold text-white">{totalArea}</p>
                  </div>
                </div>
              </div>
      ...
      <section className="rounded-xl border border-white/[0.06] bg-[#0b0d10] p-4">
        <div className="mb-4">
          <p className="text-sm font-semibold text-white">Critical Alert</p>
        </div>
        {dashboard.criticalAlerts.length > 0 ? (
          <div className="overflow-x-auto rounded-xl border border-white/[0.04] bg-white/[0.015]">
            <table className="min-w-full text-left text-sm">
              <thead>
                <tr className="border-b border-white/[0.06] bg-white/[0.02] text-[10px] uppercase tracking-[0.16em] text-white/35">
              ...
              <tbody>
                {dashboard.criticalAlerts.map((row) => (
                  <tr key={row.qcId} className="border-b border-white/[0.04] last:border-b-0">
                    <td className="px-3 py-2.5 text-white/75">{row.inspectionDate}</td>
                    <td className="px-3 py-2.5 text-white">{row.unitName}</td>
                    <td className="px-3 py-2.5 text-white/75">{row.divisionName ?? "-"}</td>
                    <td className="px-3 py-2.5 text-white/75">{row.jobName}</td>
                    <td className="px-3 py-2.5 text-red-300">{row.priorityLevel ?? "-"}</td>
                    <td className="px-3 py-2.5 text-amber-300">{row.followupStatus ?? "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
